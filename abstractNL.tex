...
Om de datastructuur van GeoSPARQL te respecteren (zie \figureref{fig:abstr_geosparql_architecture}) is gebruik gemaakt van GeoJSON. Dit is een al meer gerbuikt formaat dat ondersteuning biedt voor zowel ``Geometry'' als ``Feature'' objecten. Om nu van een ``WKT literal'' naar GeoJSON te kunnen overgaan wordt gebruik gemaakt van Terraformer. 
