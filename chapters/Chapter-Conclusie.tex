...
In dit hoofdstuk worden de resulten, besproken in \chapterref{chap:interfaces}, geïnterpreteerd. Zo wordt een antwoord geformuleerd op de vragen uit \sectionref{sec:onderzoeksvraag}.
...
Bij masterproef brengt een oplossing om de ``Linked Data publicatie''-interfaces uit te breiden met GeoSPARQL-functionaliteiten. Dit betekent dat er gelinkte data online gepubliceerd wordt. Deze data kan vervolgens opgehaald worden met behulp van SPARQL (zie \sectionref{sec:sparql}). 

De vraag is nu hoe deze opvraging kan uitgebreid worden met GeoSPARQL-functionaliteiten. Hiervoor wordt gebruik gemaakt van de al bestaande implementatie van Comunica. Door Comunica uit te breiden met deze GeoSPARQL-functionaliteiten wordt gepoogd om op dezelfde manier te kunnen werken als voordien, maar ditmaal met GeoSPARQL-functionaliteiten. Specifiek hierbij wordt de data opgehaald en gefilterd op de client zelf (zoals besproken in \chapterref{chap:interfaces}).
